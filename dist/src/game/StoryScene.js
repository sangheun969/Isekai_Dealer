...
const storyData_1 = __importDefault(require("../components/templates/storyData"));
const storyData2_1 = __importDefault(require("../components/templates/storyData2"));
class StoryScene extends phaser_1.default.Scene {
    storyTextIndex = 0;
    storyData = storyData_1.default;
    storyData2 = storyData2_1.default;
    ...
    updateUI() {
        ...
        if (this.textures.exists("gauntletItem")) {
            if (!this.gauntletItemImage) {
                this.gauntletItemImage = this.add.image(width / 2, height / 1.2, "gauntletItem");
            }
    ...
    displayStoryText(index, speechBubble) {
        if (!speechBubble) {
            console.warn("🚨 [StoryScene] speechBubble이 존재하지 않아 스토리 텍스트를 표시할 수 없음!");
            return;
        }
        ...
        if (currentStory2) {
            ...
            this.dialogueBox.setInteractive(new phaser_1.default.Geom.Rectangle(boxX, boxY, boxWidth, boxHeight), phaser_1.default.Geom.Rectangle.Contains);
    ...
    advanceStory(speechBubble) {
        if (!speechBubble) {
            console.warn("🚨 [StoryScene] speechBubble이 존재하지 않아 스토리 진행 불가!");
            return;
        }
